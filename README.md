...
To separate and manage the data lifecycle effectively, I created a structured S3 bucket where each folder represents a different stage of the workflow.  
This organization allows AWS services to work together seamlessly — the Lambda function reads from `raw/`, writes cleaned data to `processed/`, and Athena stores query results in `enriched/`.
...
**Explanation:**  
An S3 bucket named `itcs6190.assignment3.801426261.kkim43` was created with the following folder structure:

```
├── raw/        # Incoming raw data uploaded here  
├── processed/  # Filtered output generated by Lambda  
└── enriched/   # Athena query result storage  
```

This setup ensures that each component (Lambda, Glue, Athena) operates independently while sharing data through a single, well-organized bucket.

**Screenshot:**  
![Screenshot1_S3.png](./Screenshot1_S3.png)
